...
gcc_cc_s1 = Vector{Vector{uwreal}}(undef, length(enslist))
gcc_cc_s2 = Vector{Vector{uwreal}}(undef, length(enslist))
...
for (k, ens) in enumerate(ensinfo)
    ...
    BDIO_close!(fbs1)
    gcc_cc_s1[k] = res_s1["gcc_cc_disc"]
    ...
    BDIO_close!(fbs2)
    gcc_cc_s2[k] = res_s2["gcc_cc_disc"]
end
...
pi_cc_cc_s1 = [Vector{uwreal}(undef, length(Qgev)) for k in eachindex(ensinfo)]

# set 2 improvement coefficients
pi_cc_cc_s2 = [Vector{uwreal}(undef, length(Qgev)) for k in eachindex(ensinfo)]
...
for (k, ens) in enumerate(ensinfo)
    ...
    for (j,q) in enumerate(Qlat)
        pi_cc_cc_s1[k][j] =  tmr_integrand(gcc_cc_s1[k], q, KRNL, pl=true, t0ens=t0ens[k], wind=Window("SD"))
        pi_cc_cc_s2[k][j] =  tmr_integrand(gcc_cc_s2[k], q, KRNL, pl=true, t0ens=t0ens[k])
    end
end
...
popat!(ensinfo, 6)
io = IOBuffer()
write(io, "PI charm-charm disconnected. ")
fb = ALPHAdobs_create(joinpath(path_store_pi, "PIcc_disc.bdio"), io)

for (k, ens) in enumerate(ensinfo)
    extra = Dict{String, Any}("Ens" => ens.id)
    data = Dict{String, Array{uwreal}}(
        "picc_cc_s1" => pi_cc_cc_s1[k],
        "picc_cc_s2" => pi_cc_cc_s2[k]
    )
