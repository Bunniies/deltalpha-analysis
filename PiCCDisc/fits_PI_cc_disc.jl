...
#============= READ PI CHARM CHARM DISC FROM BDIO =================#
# set 1 improvement coefficients -> SD window included 
pi_cc_cc_s1 = Vector{Vector{uwreal}}(undef, 0)

# set 2 improvement coefficients -> No window included
pi_cc_cc_s2 = Vector{Vector{uwreal}}(undef, 0)

fb = BDIO_open(joinpath(path_store_pi, "PIcc_disc.bdio"), "r")
res = Dict()
count=0
while ALPHAdobs_next_p(fb)
    ...
    push!(pi_cc_cc_s1, res["picc_cc_s1"])
    push!(pi_cc_cc_s2, res["picc_cc_s2"])
...
NMOM = length(Qgev)
# set 1 picc
fitcat_cc_cc_s1 = [Vector{FitCat}(undef,0) for i=1:NMOM]
# set 2 picc
fitcat_cc_cc_s2 = [Vector{FitCat}(undef,0) for i=1:NMOM]

## no cuts in data
for s in 1:2
    xdata = [a28t0 phi2 phi4]
    if s == 1
        for q in 1:length(pi_cc_cc_s1[1])
            str = "all_data_set$(s)_q$(q)"
            # pi cc disc
            push!(fitcat_cc_cc_s1[q], FitCat(xdata, getindex.(pi_cc_cc_s1, q), str))
        end
    elseif s == 2
        for q in 1:length(pi_cc_cc_s2[1])
            str = "all_data_set$(s)_q$(q)"
            # pi cc disc
            push!(fitcat_cc_cc_s2[q], FitCat(xdata, getindex.(pi_cc_cc_s2, q), str))
        end
    end
end
##
...
for q in 1:NMOM
    ...
    for k_cat in eachindex(fitcat_cc_cc_s1[q])
        xdata = fitcat_cc_cc_s1[q][k_cat].xdata
        ydata_cc_s1 = fitcat_cc_cc_s1[q][k_cat].ydata
        ydata_cc_s2 = fitcat_cc_cc_s2[q][k_cat].ydata

        for (k_mod, model) in enumerate(f_tot_charm)
            println(k_mod)
            fit_cc_s1 = fit_routine(model, value.(xdata), ydata_cc_s1, n_par_tot_charm[k_mod], pval=true)
            fit_cc_s2 = fit_routine(model, value.(xdata), ydata_cc_s2, n_par_tot_charm[k_mod], pval=true)
            push!(fitcat_cc_cc_s1[q][k_cat].fit, fit_cc_s1)
            push!(fitcat_cc_cc_s2[q][k_cat].fit, fit_cc_s2)
...
plot_mAve_summary(cattot, xlab=vcat(label_tot_charm,label_tot_charm), charge_factor=4/9, ylab=L"$\frac{4}{9}(\Delta\alpha^{c,c})_{\mathrm{disc}}$", tt=["Set", "1-2", "LC"], path_plot=path_plot)
plot_mAve_summary(fitcat_cc_cc_s1, xlab=label_tot_charm, charge_factor=4/(9), tt=["SD Wind", "CC"],  ylab=L"$\frac{4}{9}(\Delta\alpha^{c,c})_{\mathrm{disc}}$", path_plot=path_plot)
...
for q in 1:NMOM
    ...
    ph_res_best = 4/9 * 1e5 * best_mod([0.0 phi2_ph phi4_ph], param)[1]; uwerr(ph_res_best)
    ...
    final_res, syst = 4 ./ 9 .* 1e5 .* model_average(all_res, ww_tot); uwerr(final_res)
    push!(RES, final_res)
    push!(SYST, syst)
