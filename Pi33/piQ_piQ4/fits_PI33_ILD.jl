...
cattot = [vcat(fitcat_pi33_ll_s2[k],fitcat_pi33_lc_s2[k]...) for k in eachindex(fitcat_pi33_lc_s2)]

plot_mAve_summary(cattot, xlab=vcat(label_tot_isov,label_tot_isov), charge_factor=1., ylab=ll, tt=["Set", "2", "LL-LC"], path_plot=path_plot)
...
for q in 1:NMOM
    ...
    fitcat_pi33_mean = vcat(vcat(fitcat_pi33_ll_s2[q],
                fitcat_pi33_lc_s2[q])...
    )
    ...
    final_res_all, syst =  model_average(all_res_syst, ww_tot_syst); uwerr(final_res_all)
    push!(RES, final_res)
