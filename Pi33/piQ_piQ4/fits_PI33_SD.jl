...
while ALPHAdobs_next_p(fb)
    ...
    tmp_res[extra["Ens"]] = Dict{String, Array{uwreal}}(
        "pi33_ll_SD_s1" => res["pi33_ll_SD_s1"],
        "pi33_lc_SD_s1" => res["pi33_lc_SD_s1"],
...
for (k,ens) in enumerate(enslist) # reordering data to match ensinfo order
    push!(pi_33_ll_SD_s1, tmp_res[ens]["pi33_ll_SD_s1"])
    push!(pi_33_lc_SD_s1, tmp_res[ens]["pi33_lc_SD_s1"])
    push!(pi_33_ll_SD_s2, tmp_res[ens]["pi33_ll_SD_s2"])
    push!(pi_33_lc_SD_s2, tmp_res[ens]["pi33_lc_SD_s2"])
end
...
## filter chi2 <0.6
for q in 1:NMOM
    for k_cat in eachindex(fitcat_pi33_lc_s1[q])
        for k_fit in eachindex(fitcat_pi33_lc_s1[q][k_cat].fit)
            fitcat_pi33_ll_s1[q][k_cat].fit[k_fit].chi2 = fitcat_pi33_ll_s1[q][k_cat].fit[k_fit].chi2 / fitcat_pi33_ll_s1[q][k_cat].fit[k_fit].chi2exp <0.65 ? 1000 : fitcat_pi33_ll_s1[q][k_cat].fit[k_fit].chi2
            fitcat_pi33_lc_s1[q][k_cat].fit[k_fit].chi2 = fitcat_pi33_lc_s1[q][k_cat].fit[k_fit].chi2 / fitcat_pi33_lc_s1[q][k_cat].fit[k_fit].chi2exp <0.65 ? 1000 : fitcat_pi33_lc_s1[q][k_cat].fit[k_fit].chi2
            fitcat_pi33_ll_s2[q][k_cat].fit[k_fit].chi2 = fitcat_pi33_ll_s2[q][k_cat].fit[k_fit].chi2 / fitcat_pi33_ll_s2[q][k_cat].fit[k_fit].chi2exp <0.65 ? 1000 : fitcat_pi33_ll_s2[q][k_cat].fit[k_fit].chi2
            fitcat_pi33_lc_s2[q][k_cat].fit[k_fit].chi2 = fitcat_pi33_lc_s2[q][k_cat].fit[k_fit].chi2 / fitcat_pi33_lc_s2[q][k_cat].fit[k_fit].chi2exp <0.65 ? 1000 : fitcat_pi33_lc_s2[q][k_cat].fit[k_fit].chi2
        end
    end
end
##########################
...
plot_mAve_summary(cattot, xlab=vcat(label_tot_isov,label_tot_isov,label_tot_isov,label_tot_isov), charge_factor=1., ylab=ll, tt=["Set", "1-2", "LL-LC"], path_plot=path_plot)
