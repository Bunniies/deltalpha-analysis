using Revise, HVPobs
using PyPlot, LaTeXStrings, Statistics
...
path_store_pi = "/Users/alessandroconigli/MyDrive/postdoc-mainz/projects/deltalpha/PIdata/impr_deriv/std_krnl/"
path_plot = "/Users/alessandroconigli/MyDrive/postdoc-mainz/projects/deltalpha/plots/isovector/std_krnl/"
...
fb = BDIO_open(joinpath(path_store_pi, "PI_33_std_krnl.bdio"), "r")
res = Dict()
count=0
...
## PLOTS
#########################
plot_cl_all_set(fitcat_pi33_ll_s1, fitcat_pi33_ll_s2, fitcat_pi33_lc_s1, fitcat_pi33_lc_s2, path_plot=path_plot, ylab=L"$(\Delta\alpha^{3,3})$", f_tot_isov=f_tot_isov)
plot_chiral_best_fit(fitcat_pi33_lc_s2, path_plot=path_plot, tt=["Set", "2", "LC"], f_tot_isov=f_tot_isov, ylab=L"$(\Delta\alpha^{3,3})$")
plot_cl_best_fit(fitcat_pi33_ll_s2, path_plot=path_plot, tt=["Set", "2", "LL"], f_tot_isov=f_tot_isov, ylab=L"$(\Delta\alpha^{3,3})$")
...
plot_mAve_summary(cattot, xlab=vcat(label_tot_isov,label_tot_isov,label_tot_isov,label_tot_isov), charge_factor=1., ylab=L"$(\Delta\alpha^{3,3})$", tt=["Set", "1-2", "LL-LC"], path_plot=path_plot)
...
io = IOBuffer()
write(io, "PI33 std_krnl physical results")
fb = ALPHAdobs_create(joinpath(path_phys_res, "PI33_std_krnl_physRes.bdio"), io)
for k in eachindex(RES)
    aux = RES[k] + uwreal([0.0, SYST[k]], "Syst Pi33 std_krnl")
    ALPHAdobs_write(fb, aux)
end
ALPHAdobs_close(fb)

## test reading
fb = BDIO_open(joinpath(path_phys_res, "PI33_std_krnl_physRes.bdio"), "r")
res = []
while ALPHAdobs_next_p(fb)
    d = ALPHAdobs_read_parameters(fb)
