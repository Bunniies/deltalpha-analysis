...
for q in 1:NMOM
    ...
    _, syst =  model_average(all_res_syst, ww_tot_syst)
    push!(RES, final_res)
    push!(SYST, syst)
    ...
    hist(value.(all_res_mean), bins=80, histtype="stepfilled", alpha=0.5, ec="k", color="navy", weights=ww_tot_mean, zorder=3)
