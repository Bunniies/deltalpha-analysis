...
# FVC are included, no split among different windows
# this code is speficically suited for standard kernel computation Pi(Q^2)-Pi(Q^2/4)
...
const path_store_pi = "/Users/alessandroconigli/MyDrive/postdoc-mainz/projects/deltalpha/PIdata/impr_deriv/std_krnl/"
...
for (k, ens) in enumerate(ensinfo)
    ...
    for (j,q) in enumerate(Qlat)
        pi_33_ll_s1[k][j] = tmr_integrand(g33_ll_s1[k], q, KRNL, pl=true, t0ens=t0ens[k])
        pi_33_lc_s1[k][j] = tmr_integrand(g33_lc_s1[k], q, KRNL, pl=false, t0ens=t0ens[k])
        pi_33_ll_s2[k][j] = tmr_integrand(g33_ll_s2[k], q, KRNL, pl=false, t0ens=t0ens[k])
        pi_33_lc_s2[k][j] = tmr_integrand(g33_lc_s2[k], q, KRNL, pl=false, t0ens=t0ens[k])
...
io = IOBuffer()
write(io, "PI delta 33 with standard krnl. ")
fb = ALPHAdobs_create(joinpath(path_store_pi, "PI_33_std_krnl.bdio"), io)
