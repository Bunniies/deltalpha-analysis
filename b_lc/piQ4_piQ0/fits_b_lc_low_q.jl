...
path_plot = "/Users/alessandroconigli/MyDrive/postdoc-mainz/projects/deltalpha/plots/b_lc/piQ4_piQ0/"
...
enslist = sort([ "H101", "H102", "N101", "C101",
            "B450", "D450", "D452", # "N451 and B450 removed
...
if NOERR
    for (k,ens) in enumerate(ensinfo)
        ...
        for (j,q) in enumerate(Qgev)
            ...
            pi_blc_ll_s1[k][j] *= 1.0
            pi_blc_lc_s1[k][j] *= 1.0
            pi_blc_ll_s2[k][j] *= 1.0
            pi_blc_lc_s2[k][j] *= 1.0
        end
...
plot_chiral_best_fit(fitcat_cc_lc_s1, path_plot=path_plot, tt=["Set", "1", "LC"], f_tot_isov=f_tot_charm, ylab=yll)
plot_cl_best_fit(fitcat_cc_lc_s1, path_plot=path_plot, tt=["Set", "1", "LC"], f_tot_isov=f_tot_charm, ylab=yll)
...
fb = ALPHAdobs_create(joinpath(path_phys_res, "PI_blc_physRes.bdio"), io)
for k in eachindex(RES)
