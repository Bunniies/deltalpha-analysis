...
path_store_pi = "/Users/alessandroconigli/MyDrive/postdoc-mainz/projects/deltalpha/PIdata/impr_deriv/full_q/scale_error_multimom/"
path_plot = "/Users/alessandroconigli/MyDrive/postdoc-mainz/projects/deltalpha/plots/charmConn/full_q/"
path_phys_res = "/Users/alessandroconigli/MyDrive/postdoc-mainz/projects/deltalpha/physical_results/scale_error_multimom/full_q/"
...
fb = BDIO_open(joinpath(path_store_pi, "PIcc_conn_full_q.bdio"), "r")
res = Dict()
count=0
while ALPHAdobs_next_p(fb)
    ...
    push!(pi_cc_ll_s1, res["picc_ll_s1"])
    push!(pi_cc_lc_s1, res["picc_lc_s1"])
    push!(pi_cc_ll_s2, res["picc_ll_s2"])
    push!(pi_cc_lc_s2, res["picc_lc_s2"])
...
#########################
using Statistics
yll = L"$(\Delta\alpha^{c,c})$"
plot_cl_all_set(fitcat_cc_ll_s1, fitcat_cc_ll_s2, fitcat_cc_lc_s1, fitcat_cc_lc_s2, nmom=3, path_plot=path_plot, ylab=yll, f_tot_isov=f_tot_charm)
plot_chiral_best_fit(fitcat_cc_lc_s2, path_plot=nothing, tt=["Set", "2", "LC"], f_tot_isov=f_tot_charm, ylab=yll)
...
plot_mAve_summary(cattot, xlab=vcat(label_tot_charm,label_tot_charm), charge_factor=4/9, ylab=L"$\frac{4}{9}(\Delta\alpha^{c,c})$", tt=["Set", "1-2", "LC"], path_plot=path_plot)
plot_mAve_summary(fitcat_cc_lc_s2, xlab=label_tot_charm, charge_factor=4/9, ylab=L"$\frac{4}{9}(\Delta\alpha^{c,c})$")
