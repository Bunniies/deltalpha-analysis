...
const KRNL = krnl_dα # standard full spectrum kernel
...
for (k, ens) in enumerate(ensinfo)
    ...
    for (j,q) in enumerate(Qlat)
        ...
        pi_cc_ll_s1[k][j] =  tmr_integrand(gcc_ll_s1[k], q, KRNL, pl=pl_bl, t0ens=t0ens[k])
        pi_cc_lc_s1[k][j] =  tmr_integrand(gcc_lc_s1[k], q, KRNL, pl=false, t0ens=t0ens[k])
        pi_cc_ll_s2[k][j] =  tmr_integrand(gcc_ll_s2[k], q, KRNL, pl=false, t0ens=t0ens[k])
        pi_cc_lc_s2[k][j] =  tmr_integrand(gcc_lc_s2[k], q, KRNL, pl=false, t0ens=t0ens[k])
    end
end
...
fb = ALPHAdobs_create(joinpath(path_store_pi, "PIcc_full_q.bdio"), io)
