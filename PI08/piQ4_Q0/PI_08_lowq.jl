# Compute the PI08 contribution with a given kernel and values of the virtualities 
...
for k in eachindex(ensinfo)
    ...
    # pi_08_ll_s1[k] .+= (-1/sqrt(3)*kfvc)
    pi_08_lc_s1[k] .+= (-1/sqrt(3)*kfvc)
    # pi_08_ll_s2[k] .+= (-1/sqrt(3)*kfvc)
    pi_08_lc_s2[k] .+= (-1/sqrt(3)*kfvc)
end
...
io = IOBuffer()
write(io, "PI 08. ")
fb = ALPHAdobs_create(joinpath(path_store_pi, "PI_08_beta5.bdio"), io)
...
##
#========= TEST READING =========#
fb = BDIO_open(joinpath(path_store_pi, "PI_08_beta45.bdio"), "r")
res = Dict()
