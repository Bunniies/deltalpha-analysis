...
for k in range(0, NBIN):
    ...
    isov_contrib[k] =  3 / 2 * 0.5 * Pi_light(QMVAL**2, aZ, Mz, particle_list, GG, qq, nloops, QED)
    isov_contrib[k] *= 1e5
...
for k in range(0, len(Q2LIST)):
    ...
    print("Results at ", Q2LIST[k], " GeV2")
    print("     -LO: ", 1e5 * Q2LIST[k]/ (4*QMVAL**2) * np.log(2) / 4 / np.pi**2)
    mean_tot = np.mean(isov_contrib) * Q2LIST[k] / 4 / QMVAL**2
    std_tot = np.std(isov_contrib) * Q2LIST[k] / 4 / QMVAL**2
