...
def integrand_charm(Q2, aZ, Mz, particles, mpole_on, nloops, cut_high_as3, cut_low_as3, GG):
    Q = np.sqrt(Q2)
    return 1/(12*np.pi**2) * (adler_charm_pert(aZ,Mz,Q=Q,particles=particles,mpole_on=mpole_on,nloops=nloops,cut_high_as3=cut_high_as3,cut_low_as3=cut_low_as3,GG=GG))/Q2;
...
def integrand_b33(Q2, aZ, Mz, particles, mpole_on, GG, QED, qq, cut_high_as3, cut_low_as3):
    Q = np.sqrt(Q2)
    charm = 1 / (12*np.pi**2) * (adler_charm_pert(aZ,Mz,Q=Q,particles=particles,mpole_on=mpole_on,nloops=3,cut_high_as3=cut_high_as3,cut_low_as3=cut_low_as3,GG=0.0))/Q2;
    light = 1 / (12*np.pi**2) * (adler_light_pert(aZ, Mz, Q=Q, particles=particles, GG=GG,qq=qq, nloops=5, QED=QED))/Q2;
    # return 3/2*0.5*light - charm
    return 0.5*light - charm

def pi_light_charm(Q2, aZ, Mz, particles, mpole_on, GG, qq, QED, cut_high_as3=20, cut_low_as3=0.3 ):
    return quad(integrand_b33, Q2, 4*Q2, args=(aZ,Mz,particles,mpole_on,GG,qq,QED,cut_high_as3,cut_low_as3))[0]
...
for k in range(0, NBIN):
    ...
    isov_contrib[k] = -pi_charm(QMVAL**2, aZ=aZ, Mz=Mz, particles=particle_list, mpole_on=True, nloops=5, cut_high_as3=50, cut_low_as3=0.3, GG=GG)
    isov_contrib[k] = isov_contrib[k] +  3 / 2 * 0.5 * Pi_light(QMVAL**2, aZ=aZ, Mz=Mz, particles=particle_list, GG=GG, qq=qq, nloops=5, QED=QED)
    # isov_contrib[k] = isov_contrib[k] + Pi_light(QMVAL**2, aZ=aZ, Mz=Mz, particles=particle_list, GG=GG, qq=qq, nloops=5, QED=QED)
    #isov_contrib[k] = pi_light_charm(QMVAL**2, aZ=aZ, Mz=Mz, particles=particle_list, mpole_on=False, GG=GG, qq=qq, QED=QED)



print("Results at ", QMVAL, " GeV")
