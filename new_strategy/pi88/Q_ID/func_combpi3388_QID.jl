...
phi2sq(x) =  (x[:,3] - 1.5*x[:,2]) .* x[:,2]                # phi2
a4cutoff(x)  = (x[:,3] - 1.5*x[:,2]) .* x[:,1].^(2)         # (a^2/8t0)^{2}


model_var_list = [phi42, a3cutoff, phi4sq, a4cutoff, phi2sq]
model_var_label = ["phi4-1.5phi2", "a3", "phi4", "a4", "phi2sq"]
model_map = [Bool.([i,j,k, n, l]) for i=0:1 for j=0:1 for k=0:1 for n=0:1 for l=0:1]
