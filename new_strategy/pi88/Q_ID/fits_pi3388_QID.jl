...
include("./func_combpi3388_QID.jl")
...
fb = BDIO_open(joinpath(path_store_pi, "PI_3388_Q_ID.bdio"), "r")
tmp_res = Dict()
res = Dict()
count=0
...
io = IOBuffer()
write(io, "PI3388 ID physical results")
fb = ALPHAdobs_create(joinpath(path_phys_res, "PI3388_QID_physRes.bdio"), io)
for k in eachindex(RES)
    aux = RES[k] + uwreal([0.0, SYST[k]], "Syst Pi3388 ID")
