...
logphi2(x)   = log.(x[:,2]) .- log.(value.(phi2_ph))                            #     log(Φ2) - log(Φ2_ph)
invphi2(x)   = 1 ./ x[:,2] .- 1 ./ value.(phi2_ph)

model_var_list  = [a3cutoff, a2phi2, phi2sqr, phi2log, logphi2, invphi2]
model_var_label = ["a3", "a2phi2", "phi2sqr", "phi2LogPhi2", "logPhi2", "invPhi2"]
...
for n = 2:n_par_var+1
    ...
    for (k, a) in enumerate(aux)
        ...
        if "logPhi2" ∈ model_var_label[a] && "phi2LogPhi2" ∈ model_var_label[a]
            continue
        end
        if "logPhi2" ∈ model_var_label[a] && "phi2sqr" ∈ model_var_label[a] &&  "invPhi2" ∈ model_var_label[a]
            continue
        end
        if "phi2LogPhi2" ∈ model_var_label[a] && "phi2sqr" ∈ model_var_label[a] &&  "invPhi2" ∈ model_var_label[a]
            continue
        end
