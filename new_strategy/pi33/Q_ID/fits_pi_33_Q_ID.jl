...
include("func_comb_pi33_Q_ID.jl")
...
for (k,ens) in enumerate(ensinfo)
    ...
    if ens.kappa_l == ens.kappa_s
        pi_33_ll_s1[k] .+= 1.5 * corr_pi 
        pi_33_ll_s2[k] .+= 1.5 * corr_pi 
        pi_33_lc_s1[k] .+= 1.5 * corr_pi 
        pi_33_lc_s2[k] .+= 1.5 * corr_pi 
    else
        pi_33_ll_s1[k] .+= corr_pi .+ corr_k
        pi_33_ll_s2[k] .+= corr_pi .+ corr_k
        pi_33_lc_s1[k] .+= corr_pi .+ corr_k
        pi_33_lc_s2[k] .+= corr_pi .+ corr_k
    end
end
...
for s in 1:2
    ...
    if s == 1
        for q in 1:NMOM
            str = "cuts_beta_set$(s)_q$(q)"
            # pi 33
            push!(fitcat_pi33_ll_s1[q], FitCat(xdata, getindex.(pi_33_ll_s1, q)[i_cutbeta], str))
            push!(fitcat_pi33_lc_s1[q], FitCat(xdata, getindex.(pi_33_lc_s1, q)[i_cutbeta], str))
        end
    elseif s == 2
        for q in 1:NMOM
            str = "cuts_beta_set$(s)_q$(q)"
            # pi 33
            push!(fitcat_pi33_ll_s2[q], FitCat(xdata, getindex.(pi_33_ll_s2, q)[i_cutbeta], str))
            push!(fitcat_pi33_lc_s2[q], FitCat(xdata, getindex.(pi_33_lc_s2, q)[i_cutbeta], str))
        end
    end
end
...
## test on just 1 model
xdata = fitcat_pi33_ll_s2[4][2].xdata
ydata = fitcat_pi33_ll_s2[4][2].ydata
fit_routine(f_tot_isov[29], value.(xdata), ydata, n_par_tot_isov[28], lineprint=true, ensList=enslist[i_cutphi2])
...
plot_mAve_summary(cattot, nmom=NMOM, xlab=vcat(label_tot_isov,label_tot_isov,label_tot_isov,label_tot_isov,label_tot_isov,label_tot_isov), charge_factor=1., ylab=ll, tt=["Set", "2", "LL-LC"], path_plot=path_plot)
...
fb = ALPHAdobs_create(joinpath(path_phys_res, "PI33_ID_physRes.bdio"), io)
for k in eachindex(RES)
    aux = RES[k] + uwreal([0.0, SYST[k]], "Syst Pi33 ID")
    ALPHAdobs_write(fb, aux)
end
ALPHAdobs_close(fb)

## test reading
fb = BDIO_open(joinpath(path_phys_res, "PI33_ID_physRes.bdio"), "r")
res = []
while ALPHAdobs_next_p(fb)
    d = ALPHAdobs_read_parameters(fb)
...
fitcat_pi33_syst = vcat(vcat(fitcat_pi33_ll_s1[1],
            fitcat_pi33_ll_s2[1],
            fitcat_pi33_lc_s1[1],
            fitcat_pi33_lc_s2[1])...
)
...
ww_ll_s1 = get_w_from_fitcat(fitcat_pi33_ll_s1[1])
ww_ll_s2 = get_w_from_fitcat(fitcat_pi33_ll_s2[1])
ww_lc_s1 = get_w_from_fitcat(fitcat_pi33_lc_s1[1])
ww_lc_s2 = get_w_from_fitcat(fitcat_pi33_lc_s2[1])
