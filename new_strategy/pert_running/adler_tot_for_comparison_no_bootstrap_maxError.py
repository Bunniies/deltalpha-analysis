# no bootstrap used, and instead of adding the error in quadrature when changing the input parameters individually, I change them all simultaneoysly.
...
for q in range(0,len(q_list)):
    ...
    ad_plus_all = adlertot(aZ+daZ, Mz, q_list[q], particles=particle_list_pluserr, nloops=nloops-1,mpole_on=mpole_on,mulow=mulow,GG=GG+dGG,qq=QQ+dQQ)

    # error
    error = adcentral - ad_plus_all
...
with open("/Users/alessandroconigli/MyDrive/postdoc-mainz/projects/deltalpha/running_jegerlhener/running_AdlerPy/using_my_input_values/log_spaced_mom_noBoot_maxErr/adlerPy.dat", "w") as ff:
