...
for (k, ens) in enumerate(ensinfo)
    ...
    for (j,q) in enumerate(Qlat)
        ...
        kk = KRNLsub.(x0, q)
        if j == 12
            ss = nothing #joinpath(path_plot, ens.id)
            qqq = "9"
            pplot = true
        else
            ss = nothing
        ...
        pi_3388_ll_s1[k][j] =  tmr_integrand(g3388_dlt_ll_s1[k], q, KRNLsub, pl=pplot, t0ens=t0ens[k], wind=WindSD)
        pi_3388_lc_s1[k][j] =  tmr_integrand(g3388_dlt_lc_s1[k], q, KRNLsub, pl=false, t0ens=t0ens[k], wind=WindSD)
        pi_3388_ll_s2[k][j] =  tmr_integrand(g3388_dlt_ll_s2[k], q, KRNLsub, pl=false, t0ens=t0ens[k], wind=WindSD)
        pi_3388_lc_s2[k][j] =  tmr_integrand(g3388_dlt_lc_s2[k], q, KRNLsub, pl=false, t0ens=t0ens[k], wind=WindSD)
    end
end
