...
pi_88_ll_conn_s1 = Vector{Vector{uwreal}}(undef, NENS)
pi_88_lc_conn_s1 = Vector{Vector{uwreal}}(undef, NENS)

pi_08_ll_conn_s1 = Vector{Vector{uwreal}}(undef, NENS)
pi_08_lc_conn_s1 = Vector{Vector{uwreal}}(undef, NENS)

# set 2 improvement coefficients
pi_33_ll_s2 = Vector{Vector{uwreal}}(undef, NENS)
pi_33_lc_s2 = Vector{Vector{uwreal}}(undef, NENS)

pi_88_ll_conn_s2 = Vector{Vector{uwreal}}(undef, NENS)
pi_88_lc_conn_s2 = Vector{Vector{uwreal}}(undef, NENS)

pi_08_ll_conn_s2 = Vector{Vector{uwreal}}(undef, NENS)
pi_08_lc_conn_s2 = Vector{Vector{uwreal}}(undef, NENS)
...
for (k, ens) in enumerate(ensinfo)
    ...
    pi_88_ll_conn_s1[k] = read_BDIO(hvp_path_s1[k], "dalpha", "pi_88_ll_conn")
    pi_88_lc_conn_s1[k] = read_BDIO(hvp_path_s1[k], "dalpha", "pi_88_lc_conn")

    pi_08_ll_conn_s1[k] = read_BDIO(hvp_path_s1[k], "dalpha", "pi_08_ll_conn")
    pi_08_lc_conn_s1[k] = read_BDIO(hvp_path_s1[k], "dalpha", "pi_08_lc_conn")

    # set 2 improvement coefficients
    pi_33_ll_s2[k] = read_BDIO(hvp_path_s2[k], "dalpha", "pi_33_ll" )
    pi_33_lc_s2[k] = read_BDIO(hvp_path_s2[k], "dalpha", "pi_33_lc" )

    pi_88_ll_conn_s2[k] = read_BDIO(hvp_path_s2[k], "dalpha", "pi_88_ll_conn")
    pi_88_lc_conn_s2[k] = read_BDIO(hvp_path_s2[k], "dalpha", "pi_88_lc_conn")

    pi_08_ll_conn_s2[k] = read_BDIO(hvp_path_s2[k], "dalpha", "pi_08_ll_conn")
    pi_08_lc_conn_s2[k] = read_BDIO(hvp_path_s2[k], "dalpha", "pi_08_lc_conn")
...
for s in 1:2
    ...
    if s == 1
        for q in 1:length(pi_33_ll_s1[1])
            ...
            # pi 88
            push!(fitcat_pi88_ll_s1[q], FitCat(xdata, getindex.(pi_88_ll_conn_s1, q), str))
            push!(fitcat_pi88_lc_s1[q], FitCat(xdata, getindex.(pi_88_lc_conn_s1, q), str))
            # pi 08
            push!(fitcat_pi08_ll_s1[q], FitCat(xdata, getindex.(pi_08_ll_conn_s1, q), str))
            push!(fitcat_pi08_lc_s1[q], FitCat(xdata, getindex.(pi_08_lc_conn_s1, q), str))
        end
    elseif s == 2
        for q in 1:length(pi_33_ll_s2[1])
            ...
            # pi 88
            push!(fitcat_pi88_ll_s2[q], FitCat(xdata, getindex.(pi_88_ll_conn_s2, q), str))
            push!(fitcat_pi88_lc_s2[q], FitCat(xdata, getindex.(pi_88_lc_conn_s2, q), str))
            # pi 08
            push!(fitcat_pi08_ll_s2[q], FitCat(xdata, getindex.(pi_08_ll_conn_s2, q), str))
            push!(fitcat_pi08_lc_s2[q], FitCat(xdata, getindex.(pi_08_lc_conn_s2, q), str))
        end
...
for s in 1:2
    ...
    if s == 1
        for q in 1:length(pi_33_ll_s1[1])
            ...
            # pi 88
            push!(fitcat_pi88_ll_s1[q], FitCat(xdata, getindex.(pi_88_ll_conn_s1, q)[i_cutphi2], str))
            push!(fitcat_pi88_lc_s1[q], FitCat(xdata, getindex.(pi_88_lc_conn_s1, q)[i_cutphi2], str))
            # pi 08
            push!(fitcat_pi08_ll_s1[q], FitCat(xdata, getindex.(pi_08_ll_conn_s1, q)[i_cutphi2], str))
            push!(fitcat_pi08_lc_s1[q], FitCat(xdata, getindex.(pi_08_lc_conn_s1, q)[i_cutphi2], str))
        end
    elseif s == 2
        for q in 1:length(pi_33_ll_s2[1])
            str = "all_data_set$(s)_q$(q)"
            ...
            # pi 88
            push!(fitcat_pi88_ll_s2[q], FitCat(xdata, getindex.(pi_88_ll_conn_s2, q)[i_cutphi2], str))
            push!(fitcat_pi88_lc_s2[q], FitCat(xdata, getindex.(pi_88_lc_conn_s2, q)[i_cutphi2], str))
            # pi 08
            push!(fitcat_pi08_ll_s2[q], FitCat(xdata, getindex.(pi_08_ll_conn_s2, q)[i_cutphi2], str))
            push!(fitcat_pi08_lc_s2[q], FitCat(xdata, getindex.(pi_08_lc_conn_s2, q)[i_cutphi2], str))
        end
...
for s in 1:2
    ...
    if s == 1
        for q in 1:length(pi_33_ll_s1[1])
            ...
            # pi 88
            push!(fitcat_pi88_ll_s1[q], FitCat(xdata, getindex.(pi_88_ll_conn_s1, q)[i_cutall], str))
            push!(fitcat_pi88_lc_s1[q], FitCat(xdata, getindex.(pi_88_lc_conn_s1, q)[i_cutall], str))
            # pi 08
            push!(fitcat_pi08_ll_s1[q], FitCat(xdata, getindex.(pi_08_ll_conn_s1, q)[i_cutall], str))
            push!(fitcat_pi08_lc_s1[q], FitCat(xdata, getindex.(pi_08_lc_conn_s1, q)[i_cutall], str))
        end
    elseif s == 2
        for q in 1:length(pi_33_ll_s2[1])
            str = "all_data_set$(s)_q$(q)"
            ...
            # pi 88
            push!(fitcat_pi88_ll_s2[q], FitCat(xdata, getindex.(pi_88_ll_conn_s2, q)[i_cutall], str))
            push!(fitcat_pi88_lc_s2[q], FitCat(xdata, getindex.(pi_88_lc_conn_s2, q)[i_cutall], str))
            # pi 08
            push!(fitcat_pi08_ll_s2[q], FitCat(xdata, getindex.(pi_08_ll_conn_s2, q)[i_cutall], str))
            push!(fitcat_pi08_lc_s2[q], FitCat(xdata, getindex.(pi_08_lc_conn_s2, q)[i_cutall], str))
        end
...
for q in 1:NMOM
    ...
    tight_layout()
    display(gcf())
    fname = string("cont_lim_no_LL2_q", q, ".pdf")
...
for q in 1:NMOM
    for k_cat in eachindex(fitcat_pi33_lc_s2[q])
        ...
        for (k, b) in enumerate(sort(unique(getfield.(ensinfo, :beta))))
            n_ = findall(x->x.beta == b, ensinfo[i_cutall])
...
for q in 1:NMOM
    ...
    cat_idx = Int((widx - model_idx ) / length(f_tot_isov))
    println("   widx: ", widx)
    println("   catidx: ", cat_idx)
    println("   modidx: ", model_idx)
    ## Best Res
